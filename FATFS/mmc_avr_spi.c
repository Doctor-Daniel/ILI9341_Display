...
#define CS_LOW()		PORTD &= ~_BV(PD0)						/* Set MMC_CS = low */
#define	CS_HIGH()		PORTD |= _BV(PD0)						/* Set MMC_CS = high */
#define MMC_CD			(!(PIND & _BV(PIND3)))					/* Test if card detected.   yes:true, no:false, default:true */
#define MMC_WP			PIND & _BV(PIND2)						/* Test if write protected. yes:true, no:false, default:false */
#define	FCLK_SLOW()		SPCR = _BV(SPE) | _BV(MSTR) | _BV(SPR1)	/* Set SPI clock for initialization (100-400kHz) */
#define	FCLK_FAST()		SPCR = _BV(SPE) | _BV(MSTR)				/* Set SPI clock for read/write (20MHz max) */
...
static void power_on (void)
{
	...
	DDRB |=  _BV(DDB5) | _BV(DDB7);			// _BV(DDB5) i _BV(DDB7) sa tu nadmiarowe, mozna sprobowac je usunac.
	DDRD |= _BV(DDD0);						// to jest od CS dla karty SD
	/* Enable SPI module in SPI mode 0 */
	CS_HIGH();
	FCLK_SLOW();
	SPSR = _BV(SPI2X);			// to tez chyba nie bedzie potrzebne, bo w SPI.c jest to ustawiane
}
...
static void power_off (void)
{
	...
	// SPCR = 0;					// TEGO NIE MOZE BYC, bo wylacza SPI.
	/* De-configure MOSI/MISO/SCLK/CS pins (set hi-z) */
	DDRD &= ~_BV(DDD0);		// na D0 jest podpiety CS od kart SD
}
